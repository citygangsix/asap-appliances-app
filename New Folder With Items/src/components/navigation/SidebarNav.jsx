...
export function SidebarNav({ mobileOpen, setMobileOpen }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-40 flex h-dvh max-h-dvh w-[min(300px,calc(100vw-40px))] flex-col overflow-hidden border-r border-[#2a2d36] bg-[#1c1e26] pt-[env(safe-area-inset-top)] text-white transition md:static md:h-auto md:max-h-none md:min-h-screen md:w-[272px] ${
      ...
      >
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain py-3 md:py-5">
          <div className="md:hidden">
            <NavigationGroupList groups={mobileNavigationGroups} setMobileOpen={setMobileOpen} />
          </div>
          <div className="hidden md:block">
            <NavigationGroupList groups={navigationGroups} setMobileOpen={setMobileOpen} />
          </div>
        </nav>
