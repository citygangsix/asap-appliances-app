...
export function getMockDatabaseSnapshot() {
  ...
  const technicianRows = mockTechnicians.map((technician) => ({
    ...
    primary_phone: technician.primaryPhone ?? null,
    email: technician.email ?? null,
    service_area: technician.serviceArea,
    service_zip_codes: technician.serviceZipCodes ?? [],
  ...
  const communicationRows = mockCommunications.map((entry) => ({
    ...
    call_highlights: entry.callHighlights || null,
    call_summary_sections: entry.callSummarySections
      ? {
          customer_need: entry.callSummarySections.customerNeed,
          appliance_or_system: entry.callSummarySections.applianceOrSystem,
          scheduling_and_location: entry.callSummarySections.schedulingAndLocation,
          parts_and_warranty: entry.callSummarySections.partsAndWarranty,
          billing_and_payment: entry.callSummarySections.billingAndPayment,
          follow_up_actions: entry.callSummarySections.followUpActions,
        }
      : null,
    transcription_status: entry.transcriptionStatus || null,
    transcription_error: entry.transcriptionError || null,
    transcribed_at: entry.transcriptionStatus === "completed" ? DEFAULT_TIMESTAMP : null,
